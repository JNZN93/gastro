...
export class AdminComponent implements OnInit {
  ...
  generatePdf(
    ...
    orderId: string,
    totalPrice: number,
    products: { price: number, product_article_number: string, product_id: number, product_name: string, quantity: number }[]
  ) {
    const doc = new jsPDF();
    ...
    doc.text('Lieferadresse: ' + (shippingAddress ? shippingAddress : 'Keine Angabe'), 14, 120);
  ...
  generateSelectedPdf() {
    ...
    this.orders.forEach((order, index) => {
      ...
      // Bestellnummer und weitere Infos
      const orderDateFormatted = new Date(order.order_date).toLocaleDateString(); // Datum
      const createdAtFormatted = new Date(order.created_at).toLocaleTimeString(); // Uhrzeit
